...
> **Читатель:** подключающийся — как подключить проект и чем пользоваться.
...
| `.github/workflows/rules-inbox.yml` | ежедневный прогон и ручная кнопка; тег каталога в нём **закреплён** — он закрепляет КОД действия, а выгрузка правил приходит свежей: это и есть работа входящих |
...
## Шаг второй — скажите о себе

Каталог **не ищет** потребителей и не пишет в чужие репозитории: тянет
потребитель, своим токеном и в свой трекер. Отсюда следствие, которое надо
назвать прямо: пока вы не объявились, ваш ответ по правилам никто не прочитает
— адрес неоткуда взять, — и в общей сводке «где действует» вас нет.

Правила при этом берут и без заявки: они публичны, а прогон `rules-inbox`
ведёт вашу задачу-«входящие» у вас. Заявка нужна для обратной стороны — чтобы
ваш ответ попал в сводку, а предложения правил дошли до каталога.

Объявиться: [заявка потребителя](https://github.com/ArtVsMark/Engineering-Incidents-Playbook/issues/new?template=consumer-announce.yml)
— форма, а не свободный текст: адрес ответа разбирается и становится записью
реестра, а прозу пришлось бы переносить руками, то есть шагом, который надо
помнить ([002](rules/ru/002-rule-without-mechanism.md)).

Объявленный и молчащий — находка ночного прогона, а не забытая строка: если
адрес перестанет отвечать, это увидят и скажут.
...
| `uses: ArtVsMark/Engineering-Incidents-Playbook@<!--m:ref-->v1.2.0<!--/m:ref-->` | действие «входящие»: тянет `export/rules.json`, сверяет с ответом проекта, ведёт задачу | в репозитории потребителя |
...
| `uses: ArtVsMark/Engineering-Incidents-Playbook/.github/actions/attribution@<!--m:ref-->v1.2.0<!--/m:ref-->` | сверяет авторство коммитов со списком согласованных имён | в репозитории потребителя |
