...
`gh run list --branch main --limit 30 --json name,status,conclusion,createdAt`
и `gh issue list --state open --json number,body --limit 200`. Сверка с живой
площадкой требует сети и остаётся человеку; здесь назван адрес, по которому её
делают.
