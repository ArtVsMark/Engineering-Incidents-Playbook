...
Источник подделки (правило 170): выгрузка снята с
`https://raw.githubusercontent.com/<каталог>/<ref>/export/rules.json`, ответы
трекера — с `gh issue list --state all --json number,state`. Форма ответа о
задаче снята с живого случая после отказа у потребителя: `ArtVsMark/ArtVsMark#52`.
...
# ── подъём контракта доезжает до потребителя, а не лежит полем ─────────────
#
# Замер 10 сентября: каталог публикует ШЕСТЬ номеров «чтобы сравнивали», а
# сверялся ОДИН — `bindings`. Подъём `proposals` 1.0 → 1.1 в тот же день не
# доезжал ничем: файл потребителя объявляет 1.0, и сказать ему было некому.
# Отставание `answers_to` печатал ночной прогон КАТАЛОГА в трекер КАТАЛОГА —
# находка о чужом файле ехала не адресату (142).

ЖДЁМ = {"bindings": "1.2", "proposals": "1.1", "export": "1.7"}


def test_все_три_номера_сошлись_молчание():
    assert si.contract_gaps(
        {"bindings": "1.2", "proposals": "1.1", "export": "1.7"}, ЖДЁМ) == []


def test_каждый_разошедшийся_номер_назван_отдельно():
    """Слитые в одну фразу расхождения читаются как одно, а чинятся порознь."""
    беды = si.contract_gaps(
        {"bindings": "1.1", "proposals": "1.0", "export": "1.5"}, ЖДЁМ)
    assert len(беды) == 3
    assert any("answers_to" in x and "1.5" in x for x in беды)
    assert any("предложения" in x and "1.0" in x for x in беды)


def test_одинаковое_поле_в_разных_файлах_не_путается():
    """`bindings` и `proposals` объявляют номер ОДНИМ полем `schema`.

    Первая редакция ключевала словарь по имени поля — и сверяла номер ответа
    с ожиданием для предложения. Поймано собственным прогоном.
    """
    assert si.contract_gaps({"bindings": "1.2", "proposals": "1.1"}, ЖДЁМ) == []


def test_файла_предложений_нет_это_не_находка():
    """Проект, не шлющий предложений, не обязан объявлять их формат (051)."""
    assert si.contract_gaps(
        {"bindings": "1.2", "proposals": None, "export": "1.7"}, ЖДЁМ) == []


def test_файл_есть_а_номера_нет_это_находка():
    """Отличие от отсутствия файла: несверенное не считается сошедшимся (075)."""
    (беда,) = si.contract_gaps(
        {"bindings": "1.2", "proposals": "", "export": "1.7"}, ЖДЁМ)
    assert "не объявлен" in беда


def test_каталог_не_объявил_номер_тоже_находка():
    """Третий исход: сверять нечем — это не «сходится» (039)."""
    беды = si.contract_gaps({"bindings": "1.2", "proposals": "1.1", "export": "1.7"},
                            {"bindings": "1.2"})
    assert len(беды) == 2 and all("сверить нечем" in x for x in беды)


def test_задача_называет_каждое_расхождение():
    тело = si.body_for([], [], "o/r", stale=[], total=1, answered=1,
                       started=[], nothing=[],
                       contract=["первое расхождение", "второе расхождение"])
    assert "Контракт разошёлся: 2" in тело
    assert "- первое расхождение" in тело and "- второе расхождение" in тело


def test_каждый_опубликованный_контракт_разнесён_по_спискам():
    """Механизм правила 197: опубликованное издатель использует сам.

    Каталог публикует блок `contracts` в файле, который потребитель
    скачивает, — затем, чтобы тот сверял. Замер 10 сентября: публиковалось
    шесть номеров, сверялся один, и заметить это было НЕЧЕМ. Разбор требует,
    чтобы каждый номер попал ровно в один список: либо он про файл
    потребителя и сверяется, либо про файл каталога и назван таким явно.
    Седьмой контракт, заведённый без решения, краснеет здесь.
    """
    import json
    from pathlib import Path

    опубликовано = set(json.loads(
        Path("export/rules.json").read_text(encoding="utf-8"))["contracts"])
    сверяем = {имя for имя, _, _ in si.СВОИ_КОНТРАКТЫ}
    наши = set(si.ЧУЖОМУ_НЕ_НУЖНЫ)
    assert not (сверяем & наши), "номер числится и сверяемым, и нашим"
    незакрытые = опубликовано - сверяем - наши
    assert незакрытые == set(), (
        f"опубликованы, но не отнесены ни к одному списку: {sorted(незакрытые)}")


def test_polya_answers_to_net_eto_nakhodka():
    """«Поля нет» отличается от «файла нет», и молчит только второе.

    Тернарник `свой_ответ.get(…) if свой_ответ else None` схлопывал оба факта в
    один: у пустого словаря `.get` и так даёт `None`. Живой случай — проект,
    подключившийся по заготовке до появления `answers_to`: валидный `schema`,
    версии выгрузки нет, и не сказал бы никто. Поймано ревью (075).
    """
    assert si.ответ_на_версию({"schema": "1.2"}) == ""
    assert si.ответ_на_версию({}) is None
    assert si.ответ_на_версию(None) is None
    assert si.ответ_на_версию({"answers_to": "1.5"}) == "1.5"

    (беда,) = si.contract_gaps(
        {"bindings": "1.2", "proposals": None,
         "export": si.ответ_на_версию({"schema": "1.2"})}, ЖДЁМ)
    assert "answers_to" in беда and "не объявлен" in беда


def test_fayla_otveta_net_pro_versiyu_vygruzki_molchim():
