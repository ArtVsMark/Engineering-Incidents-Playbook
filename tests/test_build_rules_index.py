...
Реализует правила каталога:
  201 — выгрузка отдаёт СУТЬ записи, а не только её имя: полнота полей `claim`
        и `applies` проверяется у КАЖДОЙ записи, иначе потребитель гадает,
        где повезло;
  043, 120 — уточнение объявляется пометкой, а обратная сторона собирается;
  005 — номера контрактов в контрактном документе переписывает сборка;
...
def test_zagotovka_pokazyvaet_kazhduyu_pometku():
    """Заготовка, которой не пользуешься сам, расходится молча (155).

    Инцидент: пометка «Уточняет»/«Refines» появилась вместе со своим гейтом и
    полем в выгрузке, а в заготовку записи не попала — при том, что зеркальная
    «Заменено» там показана с примером. Нашло это ревью, не механизм:
    `check_templates.py` спрашивает только, назван ли файл заготовки в
    `templates/README.md`, а разбор заготовки необязательных полей не требует
    намеренно.

    Здесь проверяется другое и узкое: КАЖДАЯ пометка, которую сборка читает из
    записи, названа в заготовке. Не форма примера и не текст пояснения — только
    то, что автор о пометке вообще узнает.
    """
    заготовка = Path("templates/rule-template.md").read_text(encoding="utf-8")
    пометки = {
        "Заменено": b.SUPERSEDED_RE["ru"],
        "Уточняет": b.REFINES_RE["ru"],
        "Переносится вне Claude Code": b.PORTABLE_RE["ru"],
    }
