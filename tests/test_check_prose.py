...
#
# Набор двусторонний (140): гейт обязан отвергнуть разросшуюся витрину И
# пропустить законную. Второе здесь не формальность — предел взят из правила,
# а не из достигнутого, и порог, отвергающий живую витрину, приучали бы
# обходить (051).

def test_vitrina_sverh_predela_nahodka(tmp_path, capsys):
    """Регресс, который правило само называет неизбежным, гейт называет."""
    root = repo_with(tmp_path, {"README.md": "слово " * (cp.ВИТРИНА_СЛОВ + 1)})

    assert cp.main(["--root", str(root)]) == 1
    вышло = capsys.readouterr().err
    assert "витрина выросла до" in вышло
    assert str(cp.ВИТРИНА_СЛОВ) in вышло


def test_vitrina_v_predele_chisto(tmp_path):
    """Зелёная сторона: витрина ровно по предел — ещё не находка."""
    root = repo_with(tmp_path, {"README.md": "слово " * cp.ВИТРИНА_СЛОВ})

    assert cp.main(["--root", str(root)]) == 0


def test_zhivaya_vitrina_prohodit_s_zapasom():
    """Предел не приколочен к сегодняшнему замеру: у живой витрины есть запас.

    Случай стерёг бы ровно ту поломку, ради которой храповика здесь нет: порог,
    подогнанный под текущую длину, краснел бы на первой же законной правке.
    """
    сказано = cp.prose_words((cp.ROOT / "README.md").read_text(encoding="utf-8"))

    assert сказано < cp.ВИТРИНА_СЛОВ, f"витрина уже за пределом: {сказано}"
    assert cp.ВИТРИНА_СЛОВ - сказано > 50, (
        f"запас {cp.ВИТРИНА_СЛОВ - сказано} слов — предел стал храповиком")


def test_perelivka_abzatsev_chisla_ne_menyaet():
    """Считаются СЛОВА, а не строки: перенос — форматирование (051)."""
    одной = "первое второе третье четвёртое пятое шестое"
    многими = "первое второе\nтретье\nчетвёртое пятое\nшестое"

    assert cp.prose_words(одной) == cp.prose_words(многими) == 6


def test_adres_ssylki_slovami_ne_schitaetsya():
    """Адрес не читают — и он не занимает пяти минут читателя."""
    assert cp.prose_words("[выгрузка правил](https://example.org/a/b/c.json)") == 2
    assert cp.prose_words("смотри https://example.org/очень/длинный/адрес тут") == 2


def test_poryadok_zamen_znachim_na_zhivoy_vitrine():
    """Обратный порядок съедает URL ВНУТРИ `](…)` и занижает витрину.

    НА КОРОТКОМ ПРИМЕРЕ РАЗНИЦЫ НЕ ВИДНО, и это проверено: у одиночной ссылки
    голый адрес заглатывает и закрывающую скобку, так что оба порядка дают одно
    число. Расхождение живёт на настоящем тексте, где ссылок много и за адресом
    на строке стоит ещё текст, — поэтому предмет здесь живая витрина, а не
    подделка. Замер 17.09: 833 слова в верном порядке против 611 в обратном.
    """
    текст = (cp.ROOT / "README.md").read_text(encoding="utf-8")
    наоборот = len(cp.АДРЕС_RE.sub("] ", cp.ГОЛАЯ_ССЫЛКА_RE.sub(" ", текст)).split())

    assert cp.prose_words(текст) > наоборот, (
        "порядок замен перестал быть значим — проверьте, не съедает ли "
        "голая ссылка адрес внутри скобок")


def test_granitsa_nazvana_poimyonno_i_sushchestvuet():
    """«Где потолка нет» — список, а не умолчание, и он сверен с деревом.

    Обещание границы, ни разу не проверенное, отстаёт молча: путь переименуют,
    и список будет называть то, чего нет (049).
    """
    assert cp.БЕЗ_ПОТОЛКА, "граница обязана быть названной"
    for где in cp.БЕЗ_ПОТОЛКА:
        assert (cp.ROOT / где).exists(), f"в границе назван несуществующий путь: {где}"
    assert cp.ВИТРИНА not in cp.БЕЗ_ПОТОЛКА
