...
# ── след своей записи: артефакт назван, а не подразумевается ───────────────
#
# Проверялось «известный репозиторий и хоть что-то после него», а в отказе
# обещалось «с названным артефактом, проза следом не считается». Обещание не
# держалось ничем: семь записей корпуса родились этим самым скриптом со следом
# `Owner/Repo — ADR-0010 § Контекст` и уехали к потребителю с пустым `trails`.


def test_потребитель_без_артефакта_это_находка(repo, capsys):
    tree(repo)
    assert run(repo, "--slug", "a-new-thing", "--area", "процесс", "--tier", "3",
               "--trail", "o/one — этот каталог") == 1
    assert "не разрешается" in capsys.readouterr().err


def test_потребитель_с_именем_решения_без_пути_это_находка(repo, capsys):
    """Ровно та форма, на которой обжёгся корпус."""
    tree(repo)
    assert run(repo, "--slug", "a-new-thing", "--area", "процесс", "--tier", "3",
               "--trail", "o/one — ADR-0010 § Контекст") == 1
    assert "не разрешается" in capsys.readouterr().err


def test_потребитель_с_адресом_артефакта_собирается(repo):
    """Вторая сторона: путь с расширением и корневой документ по имени проходят."""
    tree(repo)
    assert run(repo, "--slug", "a-new-thing", "--area", "процесс", "--tier", "3",
               "--trail", "o/one — `scripts/check_x.py`") == 0


# ── след предложения: приставка не безусловна ──────────────────────────────
#
# Потребитель называет артефакт в СВОИХ координатах, и репозиторий к нему
# приписать надо. Но след бывает задачей, и тогда он уже полон: приставка
# давала `Owner/Repo — Owner/Repo#181 …` — форму, которой нет ни одной
# законной, и сборка каркаса отказывала на ГОДНОМ предложении.


def test_артефакт_потребителя_получает_репозиторий():
    assert nr.след_предложения("o/one", "scripts/x.py") == "o/one — scripts/x.py"


def test_задача_приставки_не_получает():
    """Живая форма из предложения: задача с описанием следом."""
    свой = "o/one#181 — изменение на воскрешённую ветку, закрытое как дубль"
    assert nr.след_предложения("o/one", свой) == свой


def test_пустой_след_даёт_один_репозиторий():
    assert nr.след_предложения("o/one", "") == "o/one"


def test_след_задачей_с_описанием_разрешается(repo):
    """Регрессия: проверка была строже собственного корпуса.

    Заякоренная форма отвергала `владелец/репозиторий#181 — описание`, которым
    корпус пишет следы у 042 и 043. Своя же асимметрия (197).
    """
    tree(repo)
