...
@pytest.mark.parametrize("форма", [
    ...
    "---\nname: probe\ndescription: |1+\n  настоящее описание\n---\n",
        ...
        "отсечка-потом-отступ", "отступ-потом-отсечка", "буквальный-наоборот"])
...
@pytest.mark.parametrize("форма", [">0", "|0", ">23", ">-10", "|+12", ">99"],
                         ids=lambda ф: ф)
...
@pytest.mark.parametrize("форма", [">0", ">23", "|+12"], ids=lambda ф: ф)
def test_узкий_разбор_пропустил_бы_эти_формы(форма):
    """Вторая сторона замера: цена сужения названа числом, а не на слово."""
