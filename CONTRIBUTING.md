...
**Единица дробления — правило, и определяет её употребление, а не формальный
признак** ([098](rules/ru/098-the-unit-of-splitting-follows-usage.md)). Мерка
одна: **записывается то, что цитируют целиком.**

- запись, которую нельзя процитировать целиком — на неё ссылаются «первой
  половиной 143» или «вторым абзацем 068», — **дробится**: значит употребляют
  её частями, а номер один;
- **второй инцидент к существующему правилу нового номера не заводит.** Он
  дописывается в «Инцидент» второй историей. Номера не переиспользуются, и
  каждый лишний номер занимает место в общем корпусе навсегда;
- техническая проблема разбиения — «длинно», «плохо ищется» — лечится
  **метаданными**, а не дроблением: областью, ключевыми словами в указателе,
  строкой «Смежное». Разбитое ради поиска правило теряет то, ради чего
  записано, — связь утверждения с его инцидентом;
- **намеренный бандл называется бандлом.** Пара, которую всегда применяют
  вместе, остаётся одной записью, и это говорится вслух — иначе следующий
  заход разобьёт её как недосмотр.

**The unit of splitting is a rule, and usage defines it**, not a formal
criterion: what gets one number is what people quote whole. A record cited by
halves is split; a second incident of an existing rule is appended to it, never
given a new number; "too long to search" is fixed with metadata, not by
splitting.
...
python scripts/check_narrowing.py
...
python scripts/check_verdict_order.py
...
python scripts/check_text_cuts.py
python scripts/check_python_version.py
python scripts/check_exclusive.py --check
