...
| [`skill-template.md`](skill-template.md) | Навык агента, держащий правило: форма `SKILL.md`, что проверяет гейт, как объявить в ответе · An agent skill that holds a rule | [002](../rules/ru/002-rule-without-mechanism.md) · [183](../rules/ru/183-a-claim-about-the-mechanism-is-checked-against-it.md) · [022](../rules/ru/022-one-canonical-document.md) | `.claude/skills/` — навыки окна собраны по этой форме, и `scripts/check_skills.py` сверяет их разделы с ней · the window's skills follow it, and a gate checks their sections against it |
