...
> **Читатель:** подключающийся — какие заготовки есть и чем они применяются дома.
...
| [`skill-template.md`](skill-template.md) | Навык агента, держащий правило: форма `SKILL.md`, что проверяет гейт, как объявить в ответе · An agent skill that holds a rule | [002](../rules/ru/002-rule-without-mechanism.md) · [183](../rules/ru/183-a-claim-about-the-mechanism-is-checked-against-it.md) · [045](../rules/ru/045-no-silent-fallback.md) · [022](../rules/ru/022-one-canonical-document.md) | нет: навыков у каталога нет ни одного — механизм заведён контрактом 1.4 для потребителей, а ближайшие свои кандидаты (047 и 082) стоят `document_reason: impossible`, и это утверждение ещё не пересмотрено · no: the catalogue has no skills yet |
