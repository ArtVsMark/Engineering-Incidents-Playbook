...
def удаляет_ветку(command: str) -> bool:
    """Толчок УДАЛЯЕТ ссылку, а не везёт содержимое: `git push origin -d имя`.

    ПОЧЕМУ ЭТО ОТДЕЛЬНЫЙ ВОПРОС. Удаление ничего не воскрешает: коммитов оно не
    переносит вовсе, и предмет команды — ЧУЖАЯ ветка, а не текущая. Проверка
    воскрешения смотрит на текущую, и без этой развилки выходил ложный отказ на
    законной уборке: окно стоит на своей ветке, уже слитой и удалённой
    площадкой, и удаляет ПОСТОРОННЮЮ мёртвую ссылку — а сторож объявляет
    воскрешение и советует cherry-pick. Предмет отказа не совпадал с предметом
    команды.

    Поймано ревью на #468. Ложный отказ дороже пропуска (051), и здесь он ещё и
    останавливает ровно ту уборку, которую сам же сторож в отказе и просит.
    Тот же признак, что у соседа выше: `targets` уже пропускает такие команды —
    «содержимого в команде нет».
    """
    for part in without_heredocs(command).split("&&"):
        try:
            слова = shlex.split(part)
        except ValueError:
            continue
        вызов = подкоманда(слова)
        if вызов is not None and вызов[0] == "push":
            if any(w in DELETES_ALL for w in вызов[1]) or any(
                    w.startswith(":") for w in вызов[1]):
                return True
    return False
...
def main() -> int:
    ...
    if not толкает(command):
        return 0
    if not удаляет_ветку(command) and ветка_воскресает(branch):
