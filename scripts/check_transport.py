...
Реализует правила каталога:
  ...
  102 — вторая половина того же списка: послабление существует только
        ...
        бы нечего.
  059 — строка карты для самого узкого счётчика площадки: что встаёт при
        исчерпании GraphQL, известно заранее и закрыто списком, остальное
        идёт по REST и при его нуле работает.
...
def исключения() -> tuple[dict[str, str] | None, str]:
    ...
    return {и["where"]: и for и in d.get("allowed", [])}, ""
...
def main(argv: list[str] | None = None) -> int:
    ...
    # АДРЕС ИСКЛЮЧЕНИЯ ЖИВЁТ БЕЗ НОМЕРА СТРОКИ, И ЭТО ПОЧИНКА ПО ТРЕТЬЕМУ
    # СЛУЧАЮ ОДНОГО КЛАССА. Номер сдвигается любой правкой ВЫШЕ по файлу: 8
    # сентября шесть строк комментария в шапке automerge.yml уронили этот гейт,
    # а через час его же уронили новые шаги — тот же файл, тот же вызов, третий
    # номер. Тот же дефект измерен на парах правила 181, где имя «файл:строка»
    # обесценило пять записанных вердиктов разом.
    #
    # ФОРМА ТЕПЕРЬ ДВОЯКАЯ, И СТАРАЯ ОСТАЁТСЯ ЗАКОННОЙ. `файл:строка` —
    # по-прежнему точный адрес, годный там, где вызовов несколько и они разные.
    # `файл` + совпадение самой команды переживает правку: он называет ЧТО
    # разрешено, а не ГДЕ оно лежало в тот день.
    находки = []
    for где, что in предметы(args.root):
        файл = где.rsplit(":", 1)[0]
        if где in названы:
            continue
        по_файлу = названы.get(файл)
        if по_файлу is not None and (по_файлу.get("what") or что) == что:
            continue
        находки.append((где, что))
    if находки:
