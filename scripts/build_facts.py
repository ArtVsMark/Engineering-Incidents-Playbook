...
def rules(slug: str) -> tuple[dict | None, str]:
    ...
    # ДОЛИ ОБЯЗАНЫ СХОДИТЬСЯ С ЦЕЛЫМ, И `rejected` ЗДЕСЬ НЕ ЛИШНИЙ КЛЮЧ.
    # Первая редакция публиковала пять долей при total из ответа целиком:
    # 119+8+28+5+27 = 187 против total 193, и шесть отклонённых правил не
    # считал никто. Расхождение молчаливое — читатель складывает доли и не
    # получает целого, а спросить не у кого (178, 181). Контракт витрины
    # незнакомые ключи игнорирует, так что цена ключа нулевая, а цена
    # несходящейся суммы — доверие к числу.
    доли = {"gate": механизмы.get("gate", 0),
            ...
            "rejected": статусы.get("rejected", 0)}
