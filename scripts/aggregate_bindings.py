...
#: Где ответ потребителя лежит ПО СОГЛАШЕНИЮ. Все четыре подключённых стоят
#: ровно по этому адресу, и CONNECT.md заводит его же — то есть «ответа нет»
#: опровергается ОДНИМ запросом, а не ожиданием календаря.
СОГЛАШЕНИЕ = "https://raw.githubusercontent.com/{repo}/main/.rules/bindings.json"


def отвечает_по_соглашению(repo: str, читать=fetch) -> bool:
    """Лежит ли ответ по общепринятому адресу этого потребителя.

    Читатель передаётся параметром, чтобы набор не ходил в сеть: подделка
    площадки называет свой источник, а не выдумывает форму (170).
    """
    данные, _ = читать(СОГЛАШЕНИЕ.format(repo=repo))
    return bool(данные and (данные.get("rules") or {}))


def unconnected(consumers: list[dict], читать=fetch) -> list[str]:
    """Объявленные, но так и не подключившиеся — и те, кто ответил молча.
    ...
    ПРОБА ИДЁТ ПЕРЕД КАЛЕНДАРЁМ, И ЭТО ИНЦИДЕНТ, А НЕ ПРЕДОСТОРОЖНОСТЬ.
    Glossary-Python объявили потребителем 22 августа и адрес ответа не
    заполнили. Ответ он завёл — все 179 правил, схема 1.1, по общепринятому
    адресу, — но сводка показывала у него НОЛЬ, а витрина рисовала подключённым
    с нулём. Заметил это владелец глазами; механизм молчал бы ещё 17 дней, пока
    не дотикает UNCONNECTED_DAYS.
    
    Ровно правило 175: утверждение «предмета нет», опровергаемое одной
    командой, обязано быть гейтом, а не сроком. И ровно 075 с другой стороны:
    «не смотрели» показывалось как «у него ничего нет».
    """
    out: list[str] = []
    today = dt.date.today()
    for c in consumers:
        if c.get("bindings") or c.get("access") == "private":
            continue
        if отвечает_по_соглашению(c.get("repo", ""), читать):
            out.append(
                f"{c.get('repo')}: адрес ответа не заполнен, а ответ ЕСТЬ — "
                f"лежит по соглашению ({СОГЛАШЕНИЕ.format(repo=c.get('repo'))}). "
                "Сводка показывает у него ноль, и «мы не смотрели» неотличимо от "
                "«у него ничего нет» (075, 175). Заполните поле bindings")
            continue
        since = c.get("since")
        if not since:
...
#: То же самое, но на ВЕРХНЕМ уровне документа. Отметка времени сборки лежит
#: именно там, и без этого списка автоматическая пересборка заводила изменение
#: на КАЖДЫЙ толчок в общую ветку: слияние → прогон → новая отметка → ещё одно
#: изменение → снова слияние. Замер 3 сентября: два таких изменения подряд
#: (#304, #305), в каждом одна строка — новое время.
VOLATILE_TOP = ("generated_at",)
