...
Реализует правила каталога:
  ...
  190 — форма чужого ответа не хранится: в реестре лежит АДРЕС ответа
        потребителя, а не то, где у него что лежит. Недоступный ответ даёт
        находку с адресом, и сводка досчитывается по остальным — один молчащий
        потребитель не роняет её по пятерым.
...
from check_bindings import (MECHANISM_ORDER, addressed,  # noqa: E402
                            навык, навык_в_дереве, слово_держимости)
# Где лежат плагины каталога, знает гейт витрины.
...
def держание(rec: dict) -> dict:
    ...
    return {"mechanism": rec.get("mechanism") or "none",
            ...
            # Навык едет наружу адресом, как его назвал ответ. Без него
            # каталог не видел, кто держит правила навыком ЕГО плагина, —
            # а это единственный чужой механизм, который отсюда проверяем
            # (контракт ответа 1.7, сводка 1.5).
...
#: бить в ту же секунду.
ПАУЗА = 1.0


#: ЦЕНА ПОВТОРА СЧИТАЕТСЯ АРИФМЕТИКОЙ, А НЕ НА ГЛАЗ, и первая редакция считала
#: её неверно: «не больше трёх секунд» — это сумма ПАУЗ, а висеть каждая
#: попытка может до своего таймаута. Худший случай у недоступного потребителя
#: равен `ПОПЫТОК × таймаут + паузы`, и с прежними 20 секундами вышло бы 63 на
#: одного — восемь сетевых чтений ночного прогона (сводка и предложения, по
#: четыре) дали бы 8.4 минуты при бюджете работы в 10 (`consumers-sync.yml`).
#: Убитая по таймауту работа не говорит НИЧЕГО — хуже, чем красная с
#: диагнозом (158).
#:
#: Поэтому таймаут одной попытки 10 секунд: худший случай 3×10 + 1 + 2 = 33
...
#: Список читает `refresh_derived.py` — импортом, а не копией (209).
VOLATILE = ("read_at",)
...
MECHANISM_RU = {"gate": "гейт", "pipeline": "конвейер", "skill": "навык",
                "document": "документ", "process-step": "шаг процесса"}
...
#: территории расходятся молча (правило 209).
...
def stale_answers(slices: list[dict], rule_ids: list[str],
    ...
    for s in slices:
        ...
        out += unaddressed(s)
        out += навыки_плагина(s)
        out += answers_superseded(s, superseded)
    return out


def навыки_плагина_в(s: dict) -> dict[str, str]:
    """Правило → навык плагина каталога, которым его держит этот потребитель."""
    вышло: dict[str, str] = {}
    for rid, h in sorted((s.get("holds") or {}).items()):
        место = навык(h)
        if место is not None and место[0].is_relative_to(PLUGINS):
            вышло[rid] = h["skill"].strip()
    return вышло


def навыки_плагина(s: dict) -> list[str]:
    """Навык плагина каталога в чужом ответе сверяется с деревом каталога.

    ЕДИНСТВЕННЫЙ ЧУЖОЙ МЕХАНИЗМ, КОТОРЫЙ ОТСЮДА ВИДНО. Адрес
    `.claude/skills/<имя>` ведёт в дерево потребителя, и о нём мы можем
    утверждать только то, что стоит в ответе. Адрес `<плагин>:<имя>`
    (контракт ответа 1.7) ведёт в НАШЕ дерево: навык, которого у каталога нет
    — удалённый, переименованный или названный с опечаткой, — проект считает
    своим механизмом, а держит правило ничто.

    ГРАНИЦА. Поставлен ли плагин у проекта, отсюда не видно. Находка едет
    адресату, прогон от неё не краснеет — файл чужой (051, 053).
    """
    нет = []
    for rid, адрес in навыки_плагина_в(s).items():
        папка, имя = навык({"skill": адрес})
        if (чего := навык_в_дереве(имя, ROOT, папка)) is not None:
            нет.append(f"{rid} → {адрес} ({чего})")
    if not нет:
        return []
    return [f"{s['repo']}: назван навык плагина каталога, которого у каталога "
            f"нет — {'; '.join(нет)}. Правило считается держащимся навыком, а "
            "держит его ничто"]
...
def superseded_map() -> dict[str, str]:
    ...
    вторая интерпретация той же территории разошлась бы молча.
    """
    try:
...
def census(slices: list[dict], rule_ids: list[str]) -> list[str]:
    ...
    # КТО ДЕРЖИТ ПРАВИЛА НАВЫКОМ ПЛАГИНА КАТАЛОГА. Каталог держит плагин и
    # принимает навыки снизу; без этой строки он не знал бы, пользуется ли
    # кто-нибудь тем, что раздаёт. Печатается только при непустом ответе:
    # «никто» видно по её отсутствию и по сводке на диске.
    for s in slices:
        навыки = навыки_плагина_в(s)
        if навыки:
            out.append(f"  {s['repo']}: навыком плагина каталога держит "
                       f"{len(навыки)} — " + ", ".join(
...
def main() -> int:
    ...
    # НАХОДКИ О ПОТРЕБИТЕЛЕ ЕДУТ К НЕМУ, А НЕ ОСТАЮТСЯ В ЛОГЕ ПРОГОНА. До
    # этого они печатались разделом «правятся не здесь» и жили ровно там —
    # в выводе, куда потребитель не смотрит и смотреть не обязан. ЗАМЕР 8
    # сентября: 8 живых находок по четырём потребителям, и ни одна из них не
    # доехала. У находки должен быть адресат (142), а адресат чужой находки —
    # тот, кто её чинит.
    #
    # ПУБЛИКАЦИЯ, А НЕ РАССЫЛКА, и это решение владельца, принятое 7 сентября:
    # «зачем им отправлять… они просто должны свериться». Механизм, ждущий
    # уведомления, ломается вместе с каналом уведомления; сверка файла на
    # диске не ломается никак.
    по_репозиторию: dict[str, list[str]] = {}
    for строка in elsewhere:
        репо, _, текст = строка.partition(": ")
        по_репозиторию.setdefault(репо, []).append(текст or строка)
    for срез in slices:
        свои = по_репозиторию.get(срез.get("repo", ""))
        if свои:
            срез["findings"] = свои

    doc = {
        # 1.1: добавлена отметка времени. 1.2: у среза появился ключ
        # `findings` — находки каталога об ЭТОМ ответе. Прибавление поля
        ...
        #
        # 1.5: запись держания несёт `skill` — адрес навыка, как его назвал
        # ответ. Навык плагина каталога (ответ 1.7) отсюда сверяется с нашим
        # деревом, и видно, кто держит им правила.
