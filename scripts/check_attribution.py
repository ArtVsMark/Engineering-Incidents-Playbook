...
Два режима, потому что вопроса два.
  ...
  проверяется ПОЛЕ АВТОРА: подпись агента в нём означает, что squash
  припишет коммит боту. После слияния это уже долг, а не находка, поэтому в
  режиме первопредков автор не проверяется (правило 143).
...
def main() -> int:
    ...
    for rec in records:
        ...
        # Подпись агента принадлежит трейлеру, а не полю автора. Squash
        # берёт автора из коммитов ветки, и в общей ветке она окажется ботом —
        # переписать это потом нечем, ветка защищена (правила 143, 114).
        if author in names:
            findings.append(
                f"{sha[:7]} {subject[:56]}\n"
