...
Реализует правила каталога:
  192 — смена исполнителя записи в общую ветку ловится на первопредках: коммит
        без трейлера — находка, пустой диапазон после отметки — третий исход.
  ...
  135 — личность окна узнаётся ЗАПИСЬЮ, а не опросом: поле автора и
        трейлеры сверяются по тому, что уже записано. Замер 2 сентября:
        окружение сменило подпись соавтора молча, и нашла это проверка
        по первопредкам, а не память окна;
...
def sections(authors: Path) -> tuple[list[str], list[str]]:
    ...
    Разбор живёт здесь, а не у зовущего: у файла один читатель, иначе
