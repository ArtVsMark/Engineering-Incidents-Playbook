...
Два режима, потому что вопроса два.
  ...
  Первопредки    (--first-parents) — проверка ПОСЛЕ: объединяющий коммит
  составляет площадка, и в итоговой истории общей ветки атрибуции может не
  быть вовсе при зелёном гейте на каждом изменении. Починить прошлое нельзя —
  но не знать о нём хуже (правила 002, 075).
...
def first_parents(ref: str, since: str | None, names: set[str]) -> int:
    ...
    Спрашиваются только объединяющие коммиты: обычные первопредки — это прямые
    коммиты в общую ветку, и они уже проверены как коммиты ветки. Объединяющий
    коммит составляет площадка, поэтому именно он теряет трейлеры молча.
    """
    try:
    ...
    try:
        out = git("log", "--first-parent", "--merges",
    ...
    if not records:
        ...
        print(f"проверка не отработала: в {scope} нет объединяющих коммитов — "
    ...
    if missing or stranger:
        print(f"атрибуция в итоговой истории {scope}: "
              f"объединяющих коммитов {total}, без атрибуции {len(missing)}",
              file=sys.stderr)
        ...
        print("\n  Прошлое не переписать: общая ветка защищена, и это долг, а не "
              "задача (правило 114).\n  Красное здесь означает, что сообщение "
              "слияния составляется без трейлеров — чинится\n  на стороне "
              "слияния, а не правкой истории. Объявить долг и спрашивать с\n"
              "  определённого коммита — ключ --since.", file=sys.stderr)
        return 1
