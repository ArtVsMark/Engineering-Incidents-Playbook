...
Реализует правила каталога:
  ...
  076 — заготовка и навык плагина уходят к потребителю, и ссылка в них
        обязана вести туда, что есть у получателя: относительная ссылка из
        templates/ или из папки плагина в наше дерево у него мертва. Поэтому
        там стоят абсолютные ссылки на каталог, а они проверяются здесь как
        локальные — иначе умирали бы молча.
...
#: Плагин уходит к потребителю ПАПКОЙ: площадка кладёт `plugins/<имя>/` в свой
#: кеш, и ссылка жива только внутри неё. Оглавление плагина читают здесь, как
#: и оглавление заготовок. Папку плагинов называет гейт витрины (214).
from check_plugins import PLUGINS  # noqa: E402
...
def пределы(doc: Path) -> Path | None:
    """Папка, в которой ссылка из документа жива у получателя.

    None — документ остаётся здесь. Заготовку берут в чужой репозиторий папкой
    templates/, навык плагина ставят папкой своего плагина.
    """
    rel = doc.relative_to(ROOT)
    if rel.name == ОГЛАВЛЕНИЕ:
        return None
    if rel.parts[:1] == (ЗАГОТОВКИ,):
        return ROOT / ЗАГОТОВКИ
    if rel.parts[:1] == PLUGINS.parts and len(rel.parts) > 2:
        return ROOT / PLUGINS / rel.parts[1]
    return None
    ...
    for f in files:
        ...
        for m in LINK_RE.finditer(f.read_text(encoding="utf-8")):
            ...
            if raw.startswith(EXTERNAL):
                ...
                предел = пределы(f)
                if предел is not None and not path.is_relative_to(предел.resolve()):
                    problems.append(
                        f"{rel}: документ уходит к потребителю, а ссылка ведёт "
