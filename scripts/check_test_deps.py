...
#: Имя пакета — всё до первого РАЗДЕЛИТЕЛЯ: указателя версии, extras, маркера,
#: ссылки. Набор запретительный намеренно: перечислять разрешённое пришлось бы
#: алфавитом, а он у имени шире, чем кажется.
#:
#: ДВА ПРОМАХА ПОДРЯД, И ОБА ПОЙМАНЫ МЕХАНИЗМОМ, А НЕ ЧТЕНИЕМ. Первый: разбор
#: резал по списку («==», «>=», «<=», «~=», «>», «<», «[», «;»), где не было
#: «!=», и строка `pytest != 8.0` давала именем всю строку целиком; нашёл гейт
#: 195 на изменении #447 — «символьный класс перестал принимать: '!' ';' '<'
#: '=' '>'», когда прежний класс `[<>=!;]` уехал вместе со словарём имён из
#: check_runtime_deps. Второй: замена на «разрешённые символы»
#: `[A-Za-z0-9._-]+` молча выбрасывала строку целиком, если имя вне ASCII, —
#: нашли тесты, где подделка написана по-русски. Отсюда третья редакция:
#: границу задают разделители, а не алфавит.
#:
#: ГРАНИЦА. Ссылка на репозиторий (`git+https://…`) именем становится
#: целиком: разделителей в ней нет. Таких строк в файле нет, и вводить ради
#: них разбор URL значило бы чинить отсутствующее (051).
ИМЯ_ПАКЕТА = re.compile(r"[^<>=!~\[\];@\s]+")
...
def объявлено(lines: Iterable[str]) -> dict[str, set[str]]:
    ...
    при первом же пакете, объявленном только в файле (035). Теперь он зовёт
    ...
    for raw in lines:
        ...
        if not m:
            continue
        # Регистр и дефисы к одному виду: так имя сравнимо с именем в `import`.
        карта.setdefault(m.group(0).lower().replace("-", "_"), set()).update(модули)
    return карта
