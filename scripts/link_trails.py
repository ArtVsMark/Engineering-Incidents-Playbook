...
ОТДАЁТСЯ ПОТРЕБИТЕЛЮ. Названо в CONNECT.md (правило 163): механизм нужен
каждому потребителю со следами, и найти его надо на входе.
...
Исходы:
  0 — обратные ссылки на месте, ставить нечего либо всё дописано с --apply;
  1 — есть задачи без обратной ссылки, и они ОСТАЛИСЬ без неё (сухой прогон);
  2 — не отработал: экспорт не прочитан, разобрать нечем, трекер недоступен.

КОД ВОЗВРАТА НЕ МЕНЯЕТ СМЫСЛА В РЕЖИМЕ ЗАПИСИ, И ЭТО ПОЧИНКА, А НЕ УТОЧНЕНИЕ.
Прежде здесь стояло `return 1 if missing else 0`, где `missing` считался ДО
записи и после неё не очищался: с `--apply` удачная работа отвечала тем же
кодом, что и сухая находка. Читающий отличить их не мог, и прогон по расписанию
у потребителя красил успешную запись — инцидент приехал предложением от
Claude-Code_Usage-Token. Теперь считается ОСТАТОК: сколько задач осталось без
...
Реализует правила каталога:
  ...
  120 — обратный указатель «где правила действуют» СТРОИТСЯ из следов, а не
        ведётся руками: след правила дописывается в задачу потребителя;
  110 — временный каталог берётся ДО подмены PATH, восстановление в finally;
  109 — терминальный статус обязателен — 0, 1 или 2, и третий не растворяется во втором;
  186 — код возврата не меняет смысла между режимами: считается ОСТАТОК после
...
def main() -> int:
    ...
    for issue, rr in found.items():
        ...
        # ЛЮБОЙ ОТКАЗ ЧТЕНИЯ — ТРЕТИЙ ИСХОД, А НЕ «ССЫЛКА УЖЕ СТОИТ». Прежде
        # здесь терминальным считался только отказ инструмента и квоты, а код 1
        # от площадки и код 2 неразобранного ответа уходили в ветку «уже
        # сделано»: задача молча выпадала из работы, прогон зеленел (039,
        # находка обзора на #598).
        if code != 0:
    ...
    # тот же код возврата означал бы в двух режимах разное (109).
    осталось = [] if args.apply else missing
