...
PINNED = ROOT / "export" / "README.md"


def pin(tag: str) -> str | None:
    """Подставляет тег в маркеры контракта. Возвращает находку, если маркера нет."""
    text = PINNED.read_text(encoding="utf-8")
    if not PIN_RE.search(text):
        return (f"{PINNED.name}: маркера <!--m:ref--> нет. Пример подключения, "
                f"который некому переписать, вернётся к устаревшему тегу молча")
    PINNED.write_text(PIN_RE.sub(rf"\g<1>{tag}\g<2>", text), encoding="utf-8")
    return None
