...
и вызовы в значении присваивания (`X = f()` исполняет f, `X = f` — нет).
...
def вызовы_при_импорте(выражение: ast.expr) -> list[ast.Call]:
    """Вызовы, которые выражение исполняет сразу, — без тел `lambda`."""
    вышло: list[ast.Call] = []
    очередь: list[ast.AST] = [выражение]
    while очередь:
        у = очередь.pop()
        if isinstance(у, ast.Lambda):
            continue                    # тело lambda при импорте не исполняется
...
class Модуль:
    ...
    def __init__(self, имя: str, путь: Path, дерево: ast.Module, свои: set[str]) -> None:
        ...
        for узел in дерево.body:
            if isinstance(узел, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                # исполняются при импорте: декораторы, базы, умолчания
