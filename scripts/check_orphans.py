...
(в их числе `if __name__ == "__main__"`), декораторы, базы и ключевые
аргументы класса (метакласс) и тело класса, а в значении присваивания и в
умолчании аргумента — вызовы
(`X = f()` и `def g(x=f())` исполняют f, `X = f` и `def g(x=f)` — нет).
Тело lambda лениво и корнем не считается; генератор считается исполненным
целиком — ленив ли он, решает потребитель, которого разбор не видит
(граница ниже).
...
разбором не видно; такое имя называется в списке. Сирота, чей единственный
вызов лежит в генераторе, которого никто не потребил, тоже не видна: генератор
считается исполненным, чтобы не краснеть на потреблённом (051). Методы классов гейт не
...
def вызовы_при_импорте(выражение: ast.AST) -> list[ast.Call]:
    ...
    ГЕНЕРАТОР СЧИТАЕТСЯ ИСПОЛНЕННЫМ, И ЭТО ГРАНИЦА, А НЕ ЗАПЛАТКА. Ленив он
    или нет, решает тот, кто его потребит: включение (`[x for x in (f() for
    _ in y)]`), распаковка (`[*gen]`, `a, b = gen`), вызов — потребляют сразу,
    голое значение — нет. Разбор без контекста этого не знает, и третья
    находка обзора в том же месте (#614) показала, что форма за формой здесь
    не кончаются (210). Ошибки в две стороны стоят по-разному: ложная сирота —
    красное на законном коде, чего 051 не допускает; пропущенная — сирота,
    чей единственный вызов лежит в никем не потреблённом генераторе, не
    видна. Выбрано второе, и оно названо.
    """
    вышло: list[ast.Call] = []
    ...
    def корни_тела(self, тело: list[ast.stmt]) -> None:
        ...
        for узел in тело:
            if isinstance(узел, (ast.FunctionDef, ast.AsyncFunctionDef)):
                # ДЕКОРАТОР ВЫЗЫВАЕТСЯ ПРИ ОПРЕДЕЛЕНИИ — берётся целиком, как
                # вызов. УМОЛЧАНИЕ лишь ВЫЧИСЛЯЕТСЯ — это то же, что значение
                # присваивания, и разбирается тем же разбором: вызов в нём
                # исполняется, тело lambda — нет. Сырым корнем умолчание
                # `lambda: f()` держало f живой (находка обзора на #614).
                self.корни += узел.decorator_list
                for умолчание in узел.args.defaults + [у for у in узел.args.kw_defaults if у]:
                    self.корни += вызовы_при_импорте(умолчание)
            elif isinstance(узел, ast.ClassDef):
                # декоратор вызывается, база и метакласс участвуют в создании
                # класса — все берутся целиком
                self.корни += узел.decorator_list
...
def selftest() -> int:
    ...
    случаи = [
        ...
        ("умолчание lambda вычисляется сразу",
         {"a": "def f(): return 1\nX = lambda y=f(): y\n"}, {"X"}),
        ("генератор считается исполненным: ложной сироты нет",
         {"a": "def f(): return 1\nX = (f() for _ in range(2))\n"}, {"X"}),
        ("первый итерируемый генератора вычисляется сразу",
         {"a": "def f(): return []\nX = (i for i in f())\n"}, {"X"}),
        ("генератор, потреблённый включением, исполняется",
         {"a": "def f(): return 1\nX = [x for x in (f() for _ in range(1))]\n"}, {"X"}),
        ("генератор, потреблённый распаковкой, исполняется",
         {"a": "def f(): return 1\nX = [*(f() for _ in range(1))]\n"}, {"X"}),
        ("тело класса исполняется при импорте",
         {"a": "def f(): return 1\nclass A:\n    x = f()\n"}, {"A"}),
        ("тело lambda в умолчании функции не исполняется",
         {"a": "def f(): return 1\ndef g(x=lambda: f()): pass\n"}, {"f", "g"}),
        ("вызов в умолчании функции исполняется",
         {"a": "def f(): return 1\ndef g(x=f()): pass\n"}, {"g"}),
        ("декоратор-lambda вызывается при определении",
         {"a": "def f(): return 1\n@(lambda fn: f() and fn)\ndef g(): pass\n"}, {"g"}),
