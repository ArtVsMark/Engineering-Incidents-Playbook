...
ОТДАЁТСЯ ПОТРЕБИТЕЛЮ. Названо в CONNECT.md (правило 163): вынесенный
инструмент, о котором сказано только в журнале, для новичка не существует.
...
def red_names(runs: list[dict], excluded: frozenset[str]) -> list[str]:
    ...
    долг прошлой истории», — дежурный не видел настоящей красноты этой работы:
    2 сентября она покраснела, плановый прогон отчитался «ветка зелёная», и в
    ветку уехали два изменения. Долг с тех пор выражен ключом `--since` у
    самого гейта; исключение осталось от времени, когда его не было, и это
    ровно тот случай, о котором предупреждает 068 — запретительный список
