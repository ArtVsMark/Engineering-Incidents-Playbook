...
Реализует правила каталога:
  076 — задача-входящие стоит в трекере ПОЛУЧАТЕЛЯ, и путь чужого дерева в
        ней — ссылка в дерево соседа, а не голый путь, который у получателя
        читается как путь его собственного файла;
  197 — опубликованное для чужого использования издатель использует сам: все
        три номера, описывающие файлы ПОТРЕБИТЕЛЯ, сверяются, а три остальных
        названы своими явно — чтобы седьмой не завёлся несверяемым молча;
...
#: Ветка, на которой каталог публикует сводку «где действует» и картинку
#: витрины. Осиротевшая: истории кода в ней нет, только производные.
SUMMARY_BRANCH = "badges"
...
def fetch_where(catalogue: str, ref: str) -> tuple[list[dict] | None, str | None]:
    ...
    """
    # СВОДКА ЛЕЖИТ НА ВЕТКЕ `badges`, А НЕ НА ОБЩЕЙ. Это производное с одним
    # писателем, и в дереве общей ветки его нет вовсе (160): там оно давало
    # конфликт на каждом изменении, которое трогало ответ каталога о себе.
    # Ветка задана здесь, а не берётся из `ref`: `ref` — версия ПРАВИЛ, и
    # закрепиться на теге правил, потеряв свежую сводку, читатель должен
    # мочь. Смешать их значило бы связать две независимые величины (164).
    url = (f"https://raw.githubusercontent.com/{catalogue}/"
           f"{SUMMARY_BRANCH}/export/where.json")
    try:
        with urllib.request.urlopen(url, timeout=30) as resp:
...
#: Контракты, описывающие файлы САМОГО КАТАЛОГА: сверять их у потребителя
#: нечем. Список закрыт и назван здесь, а не подразумевается: разбор требует,
#: чтобы КАЖДЫЙ опубликованный номер попал ровно в один из двух списков. Иначе
#: седьмой контракт заведётся и молча не будет сверяться никем — ровно так
#: шестой и не сверялся (197).
ЧУЖОМУ_НЕ_НУЖНЫ = ("consumers", "showcase", "where")
...
def ответ_на_версию(свой_ответ: dict | None) -> str | None:
    """Версия выгрузки, по которой построены ответы: «нет поля» ≠ «нет файла».

    ТЕРНАРНИК ЗДЕСЬ НИЧЕГО НЕ ДЕЛАЛ, И ЭТО СТОИЛО НАХОДКИ. Стояло
    `свой_ответ.get("answers_to") if свой_ответ else None` — у пустого словаря
    `.get` и так возвращает `None`, так что оба факта схлопывались в один и
    сверка молчала про `export` ЦЕЛИКОМ. Молчание при отсутствующем файле было
    намеренным (о нём уже сказала находка про `schema`); молчание при
    ОТСУТСТВУЮЩЕМ ПОЛЕ — нет, и это живой случай: проект, подключившийся по
    заготовке до появления `answers_to`, объявляет валидный `schema` и не
    объявляет версию выгрузки. Ему не сказал бы никто.

    Поймано ревью на уже слитом изменении — ровно того класса, который то же
    изменение и называло своим предметом (075: несверенное не считается
    сошедшимся).
    """
    if not свой_ответ:
        return None                     # файла нет — о нём уже сказано
    return свой_ответ.get("answers_to") or ""
...
#: Ведущий адрес в `where` соседа: путь к файлу (`scripts/g.py`, `CLAUDE.md`)
#: либо образец (`.github/workflows/*.yml`). Контракт требует его разрешимым.
АДРЕС_RE = re.compile(r"^([\w.-]+(?:/[\w.*-]+)*\.[\w*]+|[\w.-]+(?:/[\w.*-]+)+)")


def адрес_у_соседа(repo: str, где: str) -> str:
    """`where` соседа, где ведущий адрес — ссылка в ЕГО репозиторий (076).

    Задача-входящие стоит в трекере ПОЛУЧАТЕЛЯ, и голый путь чужого дерева
    читается там как путь его собственного файла: «.github/workflows/review.yml»
    у проекта, у которого свой review.yml, ведёт не туда. Ссылка называет, чьё
    это дерево, и открывается. `HEAD` вместо имени ветки: какая у соседа общая
    ветка, каталог не знает и не гадает. Образец (`*`) ведёт в каталог, где он
    лежит, — файла с таким именем нет.
    """
    m = АДРЕС_RE.match(где)
    if not m:
        return f"[{repo}](https://github.com/{repo}): {где}"
    адрес = m.group(1)
    if "*" in адрес:
        папка = адрес.rsplit("/", 1)[0] if "/" in адрес else ""
        цель = f"https://github.com/{repo}/tree/HEAD/{папка}".rstrip("/")
    else:
        цель = f"https://github.com/{repo}/blob/HEAD/{адрес}"
    return f"[`{адрес}`]({цель}){где[len(адрес):]}"
...
def body_for(missing: list[dict], unreviewed: list[dict], catalogue: str,
    ...
    if solved:
        ...
        for item in solved:
            for h in item["held"]:
                где = адрес_у_соседа(h["repo"], h["where"].split(";")[0].strip())
...
def main() -> int:
    ...
    if err:
        ...
        return 2
    if not rules:
        print(f"проверка не отработала: {RULES_URL} отдал пустой список "
              "правил — сверять не с чем, а молча пропускать нельзя",
              file=sys.stderr)
        return 2
    ...
    расхождение = contract_gaps(
        {"bindings": своя_схема,
         ...
         "export": ответ_на_версию(свой_ответ)},
        ожидается)
