...
Реализует правила каталога:
  191 — фрагмент, собранный генератором вместо человека, с незаполненной
        заглушкой отвергается: запись о правиле без утверждения ушла бы в
        выпуск — так 02.09 и ушла строка «Правило 164: <…>»;
...
# выражения разъехались бы границы, а разъехавшись — молча (правило 090).
...
def added_since(ref: str) -> tuple[list[Path], str | None]:
    ...
    # РЕЖЕТСЯ ПО НУЛЮ, А НЕ ПО ПРОБЕЛУ, И ЭТО НЕ ПРИДИРКА. Ключ `-z` просили
    # ровно затем, чтобы имя с пробелом не разъехалось, — а `split()` тут же
    # это отменял и вдобавок ОСТАВЛЯЛ хвостовой `\0` в имени. Путь с таким
    # хвостом не существует ни на каком диске: `verdict_problems` пропускал
    # его первой же строкой (`not path.exists()`), и гейт печатал «фрагменты в
    # порядке», не спросив НИ ОДНОГО. Замер 8 сентября: из 38 починок 35 без
    # разбираемого вердикта — гейт стоял с заведения и не срабатывал ни разу
    # (146). Найдено внешним взглядом на изменении #405, воспроизведено
    # прогоном настоящей команды конвейера на диапазоне #404: код 0.
    return [ROOT / имя for имя in done.stdout.split("\0") if имя], None
...
#: Заглушка, которую генератор записи кладёт во фрагмент журнала. Живёт ЗДЕСЬ,
#: у гейта, а генератор берёт её импортом: рукописная копия букв разошлась бы с
#: проверкой при первой правке, и ни один тест этого не увидел бы.
ЗАГЛУШКА_ФРАГМЕНТА = "<утверждение одной фразой>"
    ...
    for path in fragments():
        ...
        if text.startswith("-"):
            problems.append(f"{path.name}: ведущий «-» подставит сборка, "
                            "в тексте он лишний")
            continue
        if ЗАГЛУШКА_ФРАГМЕНТА in text:
            problems.append(f"{path.name}: заглушка каркаса не заполнена — "
                            f"«{ЗАГЛУШКА_ФРАГМЕНТА}». Запись о правиле без его "
                            "утверждения уйдёт в выпуск, как 02.09 ушла строка "
                            "о 164 (191)")
            continue
