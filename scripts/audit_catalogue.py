...
Реализует правила каталога:
  ...
  161 — поворотный момент истории несёт четыре части и разрешимую ссылку:
        поворот без неё остаётся рассказом, а рассказ не проверить.
...
#: Сколько поворотных моментов остаётся в живом документе. Правило 108
#: требует предела, ВЫРАЖЕННОГО ЧИСЛОМ, — и вот оно. Число выбрано по
#: читаемости, а не по вкусу: столько заголовков помещается в один экран, то
#: есть столько читатель видит, не листая. Дальше документ перестаёт отвечать
#: на вопрос «что было недавно» и начинает отвечать «что было вообще».
#:
...
def check_history(root: Path) -> list[str]:
    ...
    if len(marks) > HISTORY_WINDOW:
        ...
        out.append(f"HISTORY.md: поворотов больше, чем держит окно. Перенесите "
